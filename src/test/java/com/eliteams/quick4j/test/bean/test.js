...
(function($) {
	...
	function _b(_c, _d) {
		...
		if (!_e.data("menu")) {
			_e.data("menu", {
				options : $.parser.parseOptions(_e[0], [ "width", "height" ])
			});
		}
	...
	function _f(_12, div, _13) {
		var _14 = $(div);
		var _15 = $.extend({}, $.parser.parseOptions(_14[0], [ "id", "name",
				"iconCls", "href", {
					separator : "boolean"
				} ]), {
		...
		if (!_14.hasClass("menu-sep")) {
			...
			_14.empty().append(
					$("<div class=\"menu-text\"></div>").html(_15.text));
			if (_15.iconCls) {
				$("<div class=\"menu-icon\"></div>").addClass(_15.iconCls)
						.appendTo(_14);
			}
			if (_15.id) {
				_14.attr("id", _15.id);
	...
	function _10(_17, _18) {
		...
		_18.removeClass("menu-noline")
				.addClass(_19.noline ? "menu-noline" : "");
	...
	function _3b(_3c, _3d) {
		...
		if (_40.hasClass("menu-top")) {
			...
			if (_3e + _40.outerWidth() > $(window)._outerWidth()
					+ $(document)._scrollLeft()) {
				_3e = $(window)._outerWidth() + $(document).scrollLeft()
						- _40.outerWidth() - 5;
			}
			if (_3e < 0) {
				_3e = 0;
			...
			if (_3e + _40.outerWidth() + 5 > $(window)._outerWidth()
					+ $(document).scrollLeft()) {
		...
		_40.show(0, function() {
			if (!_40[0].shadow) {
				_40[0].shadow = $("<div class=\"menu-shadow\"></div>")
						.insertAfter(_40);
			}
			_40[0].shadow.css({
			...
			if (_65) {
				...
				_65 = $.data(this, "menu", {
					options : $.extend({}, $.fn.menu.defaults, $.fn.menu
							.parseOptions(this), _63)
				});
				_2(this);
			}
			$(this).css({
				left : _65.options.left,
				top : _65.options.top
			});
		});
	};
	$.fn.menu.methods = {
		...
		setIcon : function(jq, _68) {
			return jq.each(function() {
				...
				if (_68.iconCls) {
					$("<div class=\"menu-icon\"></div>").addClass(_68.iconCls)
							.appendTo(_68.target);
				}
			});
		},
